...
'''
@File    :   ftloader_v5.py
@Time    :   2022/03/17 18:54:27
@Author  :   minskiter 
@Version :   1.0
@Contact :   minskiter@qq.com
'''
...
class FTLoaderV4(IDataLoader):
    def __init__(self, **args):
        KwargsParser(debug=True) \
            ...
            .add_argument("word_build_files", list, defaultValue=[]) \
            .parse(self, **args)

        # get cache_key
        files = [
            self.train_file, self.eval_file, self.test_file, self.tag_file
        ] + self.word_build_files
        self.cache_key = [
    ...
    def read_data_set(self):
        ...
        # ]
        # 允许自己选择需要构建的字典
        self.data_files = self.word_build_files
    ...
    def process_data(self):
        if self.use_test:
            self.myData_test = FTLoaderV5DataSet(
            ...
            self.myData = FTLoaderV5DataSet(self.train_file,
                                            self.tokenizer,
                                            self.tag_rules,
                                            self.max_seq_length,
                                            self.max_tag_length,
            ...
            if self.output_eval:
                self.myData_eval = FTLoaderV5DataSet(
                    self.eval_file,
                    self.tokenizer,
                    self.tag_rules,
                    self.max_seq_length,
...
class FTLoaderV5DataSet(Dataset):
    def __init__(self,
                 file: str,
